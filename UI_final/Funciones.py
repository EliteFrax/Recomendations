...
def Similitud_Entre_Usuarios(usuario1, usuario2, diccionario_usuarios_info_peliculas, diccionario_datos_peliculas):
    ...
    sim = sumatoria_ambos / ((sumatoria_usuario_1 * sumatoria_usuario_2)** (0.5))
    return sim
...
def Top_Ten(usuario, diccionario_usuarios_info_peliculas, diccionario_datos_peliculas, similitud_minima=50):
    #la funcion que debe retornar las 10 mejores peliculas
    print "obtener peliculas CON ranting"
    candidatos = Peliculas_Sin_Rating(usuario, diccionario_usuarios_info_peliculas, diccionario_datos_peliculas)
    print "obtenidas: ",len(candidatos),"peliculas con rating"
    print "obtener usuarios similares"
    usuarios_similares = Usuarios_Similares(usuario, diccionario_usuarios_info_peliculas, diccionario_datos_peliculas, similitud_minima)
    top_ten_previo = []
    top_ten = []
    print "obtenidos: ",len(usuarios_similares)
    print "Obteniendo lista de ratings"
    for pelicula in candidatos:
        rating = Rating_Pelicula(usuario, pelicula, usuarios_similares, diccionario_datos_peliculas)
        if rating != None:
            #print "RATING:",str(rating),"annadido a la lista!"
            top_ten_previo.append((rating, pelicula))
    print len(top_ten_previo),"ratings fueron annadidos!"
    print "LISTO!"
    top_ten_previo.sort()
    ...
    for x in range(10):
        try:
            ...
            top_ten.append((0, "SIN PELICULA"))
            continue
    print str(top_ten)
    return top_ten
    
def Usuarios_Similares(usuario, diccionario_usuarios_info_peliculas, diccionario_datos_peliculas, similitud_minima = 50):
    ...
    for usuario_comparado in similares:
        ...
        if simil >= float(similitud_minima/100.0):
            continue
        else:
